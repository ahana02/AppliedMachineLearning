...
def test_score_endpoint_non_json(client):
    response = client.post("/score", data={"text": "hello"})
...
def test_score_endpoint_exception(client, monkeypatch):

    def mock_score(*args, **kwargs):
        raise Exception("Model failure")

    monkeypatch.setattr("app.score", mock_score)
...
def test_home_form_missing_text(client):
...
def test_docker():
    # Build image
    os.system("docker build -t spam-flask-app .")

    # Run container on port 5001 (avoid conflict)
    container = subprocess.Popen(
        "docker run -p 5001:5000 spam-flask-app",
        shell=True
    )
    ...
    try:
        payload = {"text": "Congratulations! You won a lottery!"}
        ...
        container.send_signal(signal.SIGTERM)
        container.kill()